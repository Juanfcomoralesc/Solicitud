    ...
    <section id="content"><div class="ic"><div class="inner_copy">All <a href="http://www.magentothemesworld.com" title="Best Magento Templates">premium Magento themes</a> at magentothemesworld.com!</div></div>
        <div class="container_12">
          <div class="grid_12">
             ...
             echo '<dt class="color-1"><strong>Nombre: </strong>'.$reg['nombre'].'</dt>'; 
             echo '<dt class="color-1"><strong>Grupo: </strong>'.$grupo.'</dt>'; 
        ...
        { 
			 ...
			 if ($reg1['turno']=='V') 
				 ...
				 echo '<dt class="color-1"><strong>Seleccione su horario de examen: </strong><br>'; 
                     ...
                     $result = mysql_query("SELECT * FROM sessiones WHERE idsession IN(9,11,12,14,16,17,18) ORDER BY idsession ");   
			 ...
			 if ($reg1['turno']=='I') 
                 { 
					echo '<dt class="color-1"><strong>Seleccione su horario de examen: </strong><br>'; 
                     $result = mysql_query("SELECT * FROM sessiones WHERE idsession IN(9,11,14,13,16,18) ORDER BY idsession");   
                     echo '<select name="session">'; 
                     while ($row = mysql_fetch_array($result, MYSQL_ASSOC))   
                     {   
                     echo '<option value="'.$row["idsession"].' ">'.$row["dia"].' '.$row["hora"].'</option>';   
			 ...
			 if ($reg1['turno']=='M') 
                 { 
					echo '<dt class="color-1"><strong>Seleccione su horario de examen: </strong><br>'; 
                     $result = mysql_query("SELECT * FROM sessiones WHERE idsession IN(12,13,17,18) ORDER BY idsession");   
                     echo '<select name="session">'; 
                     while ($row = mysql_fetch_array($result, MYSQL_ASSOC))   
                     {   
                     echo '<option value="'.$row["idsession"].' ">'.$row["dia"].' '.$row["hora"].'</option>';   
