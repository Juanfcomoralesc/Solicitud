        ...
        { 
			 ...
			 if ($reg1['turno']=='V') 
				 ...
				 echo '<dt class="color-1"><strong>Seleccione su horario de examen: </strong><br>'; 
                     ...
                     $result = mysql_query("SELECT * FROM sessiones WHERE idsession IN(1,2,3,5,6,7,8) ORDER BY idsession");   
                     echo '<select name="session">'; // esto es lo agregado  
			 ...
			 if ($reg1['turno']=='I') 
                 { 
					echo '<dt class="color-1"><strong>Seleccione su horario de examen: </strong><br>'; 
                     $result = mysql_query("SELECT * FROM sessiones WHERE idsession IN(1,4,5,6) ORDER BY idsession");   
			 ...
			 if ($reg1['turno']=='M') 
                 { 
					echo '<dt class="color-1"><strong>Seleccione su horario de examen: </strong><br>'; 
                     $result = mysql_query("SELECT * FROM sessiones WHERE idsession IN(3,4,5,8) ORDER BY idsession");   
                     echo '<select name="session">'; 
                     while ($row = mysql_fetch_array($result, MYSQL_ASSOC))   
                     {   
                     echo '<option value="'.$row["idsession"].' ">'.$row["dia"].' '.$row["hora"].'</option>';   
